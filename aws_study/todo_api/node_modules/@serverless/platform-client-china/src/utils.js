...
if (typeof window === 'undefined') {
  ...
  const getTencentDebug = (functionInfo, region) => {
    functionInfo = typeof functionInfo === 'string' ? { functionName: functionInfo } : functionInfo;
    const key = `${functionInfo.functionName}-${region}`;
    if (!tencentDebug[key]) {
  ...
  const startTencentRemoteLogAndDebug = async (functionInfo, region, cliCallback) => {
    return await getTencentDebug(functionInfo, region).remoteDebug(cliCallback);
  };

  const stopTencentRemoteLogAndDebug = async (functionInfo, region, cliCallback) => {
    return await getTencentDebug(functionInfo, region).stop(cliCallback);
  };
